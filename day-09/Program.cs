...
namespace day_09
{
  class Program
  {
    static Dictionary<BigInteger, BigInteger> memory;
    static BigInteger pc = 0;
    static BigInteger rb = 0;
    ...
    static void Main(string[] args)
    {
      ...
      memory = input.Split(',').Select((f, i) => new { f, i }).ToDictionary(f => new BigInteger(f.i), f => BigInteger.Parse(f.f));
    ...
    static void cycle()
    {
      ...
      switch (op)
      {
        ...
        case 5: // junp-if-true
          v = read(1);
          if (!v.IsZero) { pc = read(2); } else { pc += 3; }
          break;
        ...
        case 8: // equals
          write(3, (read(1).Equals(read(2))) ? 1 : 0);
          pc += 4;
          break;
    ...
    /// <returns></returns>
    static BigInteger read(int argIdx)
    {
      string modeStr = GetOrZero(pc).ToString().PadLeft(2 + argIdx, '0');
      char modeChar = modeStr[modeStr.Length - 2 - argIdx];

      BigInteger val = GetOrZero(pc + argIdx); 
      if (modeChar == '0') val = GetOrZero(val);
      else if (modeChar == '2') val = GetOrZero(rb + val);
      return val;
    }

    static void write(int argIdx, BigInteger value)
    {
      string modeStr = GetOrZero(pc).ToString().PadLeft(2 + argIdx, '0');
      char modeChar = modeStr[modeStr.Length - 2 - argIdx];
      ...
      BigInteger addr = GetOrZero(pc + argIdx);
      if (modeChar == '2') addr = rb + addr;

      if (!memory.ContainsKey(addr)) memory.Add(addr, 0);
      memory[addr] = value;
    }

    static BigInteger GetOrZero(BigInteger addr)
    {
      if (memory.TryGetValue(addr, out BigInteger result))
      {
        return result;
      }
      return 0;
